...
class ActivityIndexPage(CFGOVPage):
    ...
    header_sidebar = StreamField([("image", TdpSearchHeroImage())], blank=True)
    ...
    def can_create_at(cls, parent):
        ...
        return (
            super(ActivityIndexPage, cls).can_create_at(parent)
    ...
    def get_template(self, request):
        ...
        if "partial" in request.GET:
            template = "teachers_digital_platform/activity_search_facets_and_results.html"  # noqa
        return template
    ...
    def get_context(self, request, *args, **kwargs):
        facet_map = (
            ...
            (
                ...
                (ActivityCouncilForEconEd, False, 25),
            ),
        )
        ...
        payload.update({"results": results, "total_results": total_results})
    ...
    def get_all_facets(
        self, facet_map, sqs, facet_counts, facet_queries, selected_facets
    ):
        all_facets = {}
    ...
    def get_flat_facets(self, class_object, narrowed_facets, selected_facets):
        final_facets = [
            ...
            for result in class_object.objects.filter(
                pk__in=narrowed_facets
            ).values("id", "title")
        ]
        return final_facets

    def get_nested_facets(
        self, class_object, narrowed_facets, selected_facets, parent=None
    ):
        if not parent:
            flat_final_facets = [
                {
            ...
            root_facets = [
                root_facet
                for root_facet in flat_final_facets
                if root_facet["parent"] is None
            ]
            ...
            children = [
                {
                    "selected": result["id"] in selected_facets
                    or result["parent"] in selected_facets,
                    "id": result["id"],
                    "title": result["title"],
                    "parent": result["parent"],
                    "children": self.get_nested_facets(
                        class_object,
                        narrowed_facets,
                        selected_facets,
                        result["id"],
                    ...
                    "child_selected": any(
                        child["selected"] is True
                        or child["child_selected"] is True
                        for child in self.get_nested_facets(
                            class_object,
                            narrowed_facets,
                            selected_facets,
                            result["id"],
...
class ActivityPage(CFGOVPage):
    ...
    activity_duration = models.ForeignKey(
        ActivityDuration, blank=False, on_delete=models.PROTECT
    )
    # Standards taught
